...
The Data Reuse Analysis (DRA) and the MHLA steps have been integrated in the robust Memory Hierarchy (MH) tool based on the ATOMIUM framework.
...
In the memory allocation and signal-to-memory assignment step, arrays were assigned to physical memories or to banks within predefined memories. 
However, the arrays are still multi-dimensional, while the memory itself knows only addresses. 
In other words, the physical address for every signal element still has to be determined. 
