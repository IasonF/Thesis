...
In \cite{4b},  cache partitioning, a layout technique for arrays, maps each array to different cache partitions to reduce conflicts. 
In \cite{5b} authors address the problem for cache miss reduction by evaluating the tiling size for arrays and merging the arrays appropriately for each loop nest. 
Memory Hierarchy Layer Assignment (MHLA), an optimization methodology for data memory hierarchy presented in \cite{6b}, determines for each data set an appropriate layer in the hierarchy and type of memory (single/dual port) taking data re-use into account. 
The strategy in \cite{7b} partitions the variables to scratch-pad memory and DRAM to minimize interference between variables. 
 The cache-oriented techniques proposed earlier by researchers in \cite{8b} do not find a straightforward application in our context, where the target system hardware consists of a scratch-pad memory and a SIMD functional unit.
...
This processor belongs to the class of coarse-grain reconfigurable array (CGRA) \nomenclature{CGRA}{coarse-grain reconfigurable array} processors and is described in more detail in \cite{lee2003compilation}.
...
The width of the FU \nomenclature{FU}{functional unit} corresponds to the number of pairs of array elements on which the specific instruction is performed.
...
\item Successive Over Relaxation (SOR) \nomenclature{SOR}{successive over relaxation} is a method for evaluating partial differential equations or solving a linear system of equations.
...
\item FFT \nomenclature{FFT}{fast Fourier transformation} benchmark has an irregular pattern during the access of the pilot matrices. 
...
%\begin{tabular}{c c}
%\includegraphics[width=0.45\linewidth]{Images/Example.pdf} & \includegraphics[width=0.45\linewidth]{Images/sor.pdf} \\
